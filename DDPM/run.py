...
def run(args):
    ...
    #DATA
    print('getting data...')
    if config.train_data == 'mnist':
        from mnist_dataset import MnistDataset
        train_data = MnistDataset(config.train_data_config)
    elif config.train_data == 'flower':
    ...
    show_or_save_batch_img_tensor(batch['img'], int(math.sqrt(batch['img'].shape[0])), denorm=True, mode='save', save_p=os.path.join(config.ckp_root, 'train_data_batch.png'))
